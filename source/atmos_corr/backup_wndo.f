...
C
      ANV=934.6           !GOES-I IMAGER @10.7MICRONS
...
C
       ...
       do ia=1,ilg
        ...
        if (iflag(ia,ja).ge.1) then
          tc15(ia,ja)=-9999.
          tc55(ia,ja)=-9999.
          pw(ia,ja) = -9999.
          goto 299
        endif
        sfpr=pres(ipt,jpt,30)/100.          !P in millibars
        if (sfpr.eq.0.0) goto 299

        do i=1,kz
!          ka=kz-i+1
          ka=i
          PI(i)=pres(ipt,jpt,(kz-ka)+1)/100.                 !millibars
          TA(i)=theta(ipt,jpt,(kz-ka)+1)!/(1000/PI(i))**0.286
          EI(i)=spfh(ipt,jpt,(kz-ka)+1)
       if (ia.eq.251.and.ja.eq.381) then
        write(6,*) sfpr, PI(ka),EI(ka)*1000.,TA(ka)
       endif
        enddo
  ...
  140 CONTINUE
      SURT(1)=radini(ia,ja)
      SURT(2)=radfin(ia,ja)
!       if (ia.eq.176.and.ja.eq.356) then
!        write(6,*) radini(ia,ja), radfin(ia,ja) 
!       endif
      if (SURT(1).eq.0.0 .or. SURT(2).eq.0.0) then
        tc15(ia,ja)=BADATA
        tc55(ia,ja)=BADATA
        radini(ia,ja)=BADATA
        radfin(ia,ja)=BADATA
...
cmca      CORR=SURT(I)-TRAD
     ...
     &         /trans
      ...
      corr=trad-surt(i)
      if (ia.eq.251  .and. ja.eq.381) then
        print*,'trad corr epsln grndrad ',trad,corr,epsln,grndrad
      endif
C
      if (I.eq.1) then
...
ccc     radini(ia,ja)=radini(ia,ja)+CORR    !Apply correction (radini)
        rdini=radini(ia,ja)+CORR            !Apply correction (radini)
      endif
      if (I.eq.2) then
...
C
      if (ia.eq.391.and.ja.eq.114) then
       WRITE(6,740) SURT(I),TRAD,CORR,ANGLE
  740 FORMAT(/2X,'  SURFACE T.',F8.2,'     SATELLITE',F8.2,
     X'      SURF.-SAT. TEMP',
     X F6.2,10X,'ANGLE',F8.1)
       endif
  760 CONTINUE

      if (ia.eq.391.and.ja.eq.114) then

       WRITE(6,800) SLOPE,OFFSET
  800 FORMAT(/12X,'T(SATELLITE)=',F10.3,'  T(GROUND) + ',F10.2)
       WRITE(6,860) RSLOPE,ROFF
  860 FORMAT(/12X,'T(GROUND)=',F10.3,'  T(SATELLITE) + ',F10.2)
       endif
C
