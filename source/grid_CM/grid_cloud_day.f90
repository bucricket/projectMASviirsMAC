...
do i = 2, gx-2
 if (glat(i,j+1).gt.ilat.and.glat(i,j).gt.ilat.and.glon(i+1,j).gt.ilon.and.glon(i,j).gt.ilon) then
  ...
  if (ipoint1.le.nx-1.and.ipoint2.le.nx-1.and.jpoint1.le.ny-1.and.jpoint2.le.ny-1) then
   do n = jpoint1, jpoint2
   do m = ipoint2+1, ipoint1+1
    if (m.eq.3000.and.n.eq.3000) then
     write(6,*) i, j, temp(i,j)
    endif
...
if (npix.gt.0) then
!write(i1,'(a,a,a)') '/raid1/sport/people/chain/VIIRS_PROCESS/grid_CM/cloud_sum1_',arg7,'.dat'
!write(i2,'(a,a,a)') '/raid1/sport/people/chain/VIIRS_PROCESS/grid_CM/cloud_count1_',arg7,'.dat'
